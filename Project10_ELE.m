...
R = 8.314; %J/mol-K
F = 96485; %C/mol
DH2H2O = (3.8378e-3)/3; %m2/s
DO2N2 = (2.9417e-4)/3; %m2/s
L_GDLa = 5e-3; %m
L_GDLc = 5e-3; %m
L_YSZ = 0.5e-3; %m
i_eqc = 1000; %A/m2
...
title('1000 C, 3 bar, 1 m/s, Electrolyte = 0.5 mm')
xlim([0 140])
ylim([0 1.75])
hold off
plotfixer
...
plot([0 Distance],[0;(Current(:,53)/Acell)]/1000)
plot([0 Distance],[0;(power(:,53)/Acell)]/1000)
plot([0 Distance(1:end-1)],[0;(-HeatTransfer(:,53)/Acell)]/1000)
legend('Current Density (kA/m^{2})','Elec. Power Density (kW/m^{2})','Heat Flux (kW/m^{2})')
title('1000 C, 3 bar, 0.52 V, Electrolyte = 0.5 mm')
...
hold on
Distance = L*(1:1:Cellnum)/Cellnum;
plot(Distance,xH2(:,53))
plot(Distance,xO2(:,53))
plot(Distance,xH2O(:,53))
plot(Distance,pot_diff_eq(:,53))
plot([0 0.5],[0.52 0.52],'--k')
